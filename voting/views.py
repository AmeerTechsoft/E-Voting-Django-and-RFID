...
def verify(request):
    # You can remove this view entirely, as it won't be needed without OTP verification
    context = {
        'page_title': 'Card UID Verification'
    }
    return render(request, "voting/voter/verify.html", context)

def verify_card_uid(request):
    if request.method == 'POST':
        card_uid = request.POST.get('card_uid')
        user = request.user
        if user.voter.card_uid == card_uid:
            user.voter.verified = True
            user.voter.save()
            messages.success(request, "You are now verified. Please cast your vote")
            return redirect(reverse('show_ballot'))
            
        else:
           messages.error(request, "Card is Incorrect")
           context = {'page_title': 'Card UID Verification'}
           return render(request, "voting/voter/verify.html", context)
    else:
        messages.error(request, "Invalid Credentials")
        return render(request, "voting/voter/login.html")
